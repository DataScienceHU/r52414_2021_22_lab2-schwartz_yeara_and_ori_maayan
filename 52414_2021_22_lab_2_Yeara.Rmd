...
### section a
```{r}
start_point <- str_locate_all(txt_moby, regex("CHAPTER \\d. Loomings."))
start_point$end
start_point <- str_locate(txt_moby, regex("CHAPTER\\s\\d+\\.\\s[\\w\\s]*\\.$"))

all_chap <- str_sub(txt_moby, 35376, 38000) #nchar(txt_moby))
chap_vec <- unlist(strsplit(all_chap, split = "CHAPTER", fixed=TRUE))
dat_chap <- data.frame("chapter" =(1:128), "chap_content" = chap_vec)
dat_moby$length <- as.numeric(nchar(dat_moby$word))
```
